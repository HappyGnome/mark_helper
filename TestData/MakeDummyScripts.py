...
# 15, 15, 15
# {question_name:[part_max_marks]}
questions = {'1': {'a': 2, 'b': 3, 'c': 5, 'd': 5},
             '2': {'a': 8, 'b': 2, 'c': 5},
             '3': {'a': 3, 'b': 3, 'c': 4, 'd': 5}}
paper_width = 21
paper_height = 29.7
...
for s in stu_nums:
    ...
    with open(filepath, 'w') as f:
        ...
        # position (as proportion of a page) in script file modulo 1 page
        at = 0
        ...
        for q in questions:
            ...
            for part in questions[q]:
                if random.random() > prob_miss_part:
                    ...
                    at = skip_pages(at, f)
                    f.write("\\mnoth{{{}}}{{{}}} {{({})}}\n"
                            .format(0.2*paper_width,
                                    at2actual(at)*paper_height, part))
                    at += random.uniform(*minmax_pages_per_q_part)
                    at = skip_pages(at, f)
                    part_mark = int(random.random()*questions[q][part])
                    mark += part_mark
                    f.write("\\mnoth{{{}}}{{{}}} {{...Work worth {} marks}}\n"
                            .format(random.uniform(0.1*paper_width,
                                                   0.8*paper_width),
                                    at2actual(at)*paper_height, part_mark))
            stu_marks[s][q] = mark
        f.write("\\end{mpage}\n\\end{document}")
...
# save correct marks
with open(os.path.join('DummyScripts', 'mark_check.csv'), 'w') as f:
    f.write("Student #")
    for q in questions:
        f.write(",{}".format(q))
